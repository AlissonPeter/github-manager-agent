...
**Descrição:** Implementação do loop contínuo de leitura no terminal para o usuário interagir com o agente e da ferramenta real de integração com a API REST do GitHub para criação, edição e exclusão de issues, utilizando o `GITHUB_TOKEN` e o `GITHUB_REPO` definidos no arquivo `.env.example`.
...
- [ ] Configurar `GITHUB_REPO` no `.env` como repositório padrão e implementar fallback automático no `executor_node`.
- [ ] Implementar `enhancer_node` que melhora descrições e cria checklists via Ollama antes da criação da issue.
...
**Descrição:** Remover dependência da API do Google Gemini (`google-genai`) em favor do Ollama local (`ollama`), eliminando custos de API e problemas de quota.
...
### Checklist de Entrega:
- [ ] Substituir dependência `google-genai` por `ollama` no `requirements.txt`.
- [ ] Atualizar `router_node` para usar `ollama.chat()` em vez do cliente Gemini.
- [ ] Remover variável `GEMINI_API_KEY` do `.env` e `.env.example`.
- [ ] Adicionar variável `OLLAMA_MODEL` no `.env` e `.env.example`.
