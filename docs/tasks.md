...
## Critérios de Aceitação
- [x] Adicionar comando `histórico` (ou `historico`) à lista de comandos válidos.
- [x] Manter um registro em memória de todas as ações realizadas sobre issues (criar, editar, fechar).
- [x] Exibir o histórico formatado com: título da issue, número da issue e ação realizada.
- [x] Exibir as issues na ordem cronológica (mais recente primeiro).
- [x] Caso não haja histórico, exibir mensagem informativa.
- [x] Criar testes unitários para validar o registro e exibição do histórico.

---

## T-013: Falta de tratamento de erro quando Ollama está offline
## Descrição
